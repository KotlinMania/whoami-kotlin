# Claude Code Project Instructions — whoami-kotlin

## Project Overview

This is **whoami-kotlin**, a clean-room Kotlin Multiplatform port of the upstream Rust crate [`whoami`](https://crates.io/crates/whoami).

The upstream Rust source is the read-only translation oracle. When porting begins, clone it into `tmp/whoami/` (gitignored). **Never edit `tmp/`.** If upstream looks wrong, the bug is in the port or in your understanding of Rust, not in `tmp/`.
...
```kotlin
// port-lint: source <path-relative-to-tmp/whoami>
